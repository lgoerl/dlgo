...
class LocalGtpBot:
    ...
    def __init__(self, go_bot, termination=None, handicap=0,
        ...
        self.gtp_stream = subprocess.Popen(
            cmd, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, bufsize=0,
        )
        # print(self.gtp_stream.stderr.read())
    ...
    def get_response(self):
        for line in iter(self.gtp_stream.stdout.readline, b''):
            line = line.decode()
            if line[0] == '=':
                # print('White plays', line)
                result = re.sub('^= ?', '', line.strip())
                break
        return result

    def get_response0(self):
        succeeded = False
        result = ''
        while not succeeded:

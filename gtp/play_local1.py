import h5py, re, subprocess, os

from dlgo.agent.dlagent import load_prediction_agent
...
class LocalGtpBot:
    def __init__(self, go_bot, termination_strat=None, handicap=0, opponent='gnugo', output_sgf='out.sgf', our_color='b'):
        self.bot = TerminationAgent(go_bot, termination_strat)
    ...
    def opponent_cmd(opponent):
        if opponent == 'gnugo':
            ...
            raise ValueError('Unknown opponent name')
    ...
    def run(self):
        print('running. setting boardsize')
        self.command_and_response("boardsize 19\n")
        print('setting handicap')
        self.set_handicap()
...
if __name__ == "__main__":
    print(os.listdir('./'))
    print('loading agent')
    bot = load_prediction_agent(h5py.File("dlgo/agent/latest.hdf5", "r"))
    print('instanciating LocalGtpBot')
    gnu_go = LocalGtpBot(go_bot=bot, termination_strat=PassWhenOpponentPasses(),
...
# cmd=["pachi"]
# pipe = subprocess.PIPE
# stream = subprocess.Popen(cmd, stdin=pipe, stdout=pipe)
# stream.stdin.write("boardsize 19\n".encode('utf-8'))

...
class IllegalMoveError(Exception):
    def __init__(self, player, point, check):
        self.player = player
        self.point = point
        self.check = check

    def __repr__(self):
        return f'{repr(self.player)} attempting play at {repr(self.point)}; {repr(self.check)}'
...
class Board():
    ...
    def place_stone(self, player, point):
        if not self.is_on_grid(point):
            raise IllegalMoveError(player, point, 'not on grid')
        if self._grid.get(point) is not None:
            raise IllegalMoveError(player, point, 'grid.get() is not None')
        adjacent_same_color = []
        ...
        for string in adjacent_opposite_color:
    ...
    def _is_self_capture(self, player, point):
        friendly_strings = []
        for neighbor in self.neighbor_table[point]:
            neighbor_string = self._grid.get(neighbor)
            if neighbor_string is None:
        ...
        if all(string.num_liberties == 1 for string in friendly_strings):
            return True
        return False

    def will_capture(self, player, point):
        for neighbor in self.neighbor_table:
            string = self._grid.get(neighbor)
            if string is None:
                continue
            if string.color == player:
                continue
            else:
    ...
    def __deepcopy__(self, memodict={}):
        ...
        copied._hash = copy.copy(self._hash)
        return copied
...
class GameState():
    ...
    def is_move_self_capture(self, player, move):
        if not move.is_play:
            return False
        return self.board._is_self_capture(player, move.point)
